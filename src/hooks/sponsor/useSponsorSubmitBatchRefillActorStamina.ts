...
import type {
  BatchRefillActorStaminaResult,
  ExecuteBatchRefillActorStaminaParams,
} from '@/hooks/solana/useSubmitBatchRefillActorStamina';
...
/**
 * 邮箱登录 · 批量补充体力代付：fee payer / sponor = spender，用户 signMessage，POST sponsor。
 */
export function useSponsorSubmitBatchRefillActorStamina() {
  ...
  const executeSponsorBatchRefillActorStamina = async (
    params: ExecuteBatchRefillActorStaminaParams,
  ): Promise<BatchRefillActorStaminaResult> => {
    const logPrefix = '[useSponsorSubmitBatchRefillActorStamina]';

    if (!isEmbeddedLogin) {
      throw new Error(
        'Sponsor batch refill is only available for email login users',
      );
    }
    ...
    const { refillIx, actorAssetPdas, payerPayAccount, kitFixedAccountCount } =
    ...
    console.log(`${logPrefix} batch_refill accounts`, {
    ...
    const txHash = await submitSponsorTransaction(
      ...
      'Sponsor batch_refill_actor_stamina request failed',
    );
  ...
  return {
    isReady,
    executeSponsorBatchRefillActorStamina,
  };
}

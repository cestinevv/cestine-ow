/**
 * 叙述者中心 — 「删除角色」二次确认弹窗。
 * 备注：角色管理内删除入口；确认后由父组件调用 deleteActor。
 */
...
export type DeleteActorConfirmDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  /** 确认删除：由父组件调用 DELETE `/api/mini-drama/creator/actors/{id}` */
  onConfirmDelete: () => void | Promise<void>;
  isDeleting: boolean;
};

export function DeleteActorConfirmDialog({
  open,
  onOpenChange,
  onConfirmDelete,
  isDeleting,
}: DeleteActorConfirmDialogProps) {
